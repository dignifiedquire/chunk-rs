...
impl Rabin {
    pub fn new(config: Config, preset: Preset) -> Self {
        ...
        let init_state = ((preset.out_table[0] << 8) | 1)
            ^ (preset.mod_table[preset.out_table[0] as usize >> 45]);
    ...
    pub fn split<'a, 'b>(&'a self, buf: &'b [u8], use_entire_buffer: bool) -> Split<'a, 'b> {
        Split {
            rabin: self,
            state: 0,
            cur_idx: 0,
...
impl<'a, 'b> Iterator for Split<'a, 'b> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.next_round_max = self.last_idx + self.rabin.config.max_size;

        // we will be running out of data, but still *could* run a round
        if self.next_round_max > self.post_buf_idx {
            // abort early if we are allowed to
            if !self.use_entire_buffer {
                self.done = true;
                return None;
            }
            // otherwise signify where we stop hard
            self.next_round_max = self.post_buf_idx;
        }

        // in this case we will *NOT* be able to run another round at all
        if self.cur_idx + self.rabin.config.min_size >= self.post_buf_idx {
            self.done = true;
            if self.use_entire_buffer && self.post_buf_idx != self.cur_idx {
                // TODO: verify this matches chunker.Chunk{Size: postBufIdx - curIdx})
                let (chunk, rest) = self.buf.split_at(self.post_buf_idx - self.cur_idx);
                self.buf = rest;
                return Some(chunk);
            }
            return None;
        ...
        // reset
        self.state = self.rabin.init_state;

        // preheat
        self.cur_idx += self.rabin.mins_sans_preheat;
        for i in 1..=self.rabin.preset.window_size {
            if i == self.rabin.preset.window_size {
                self.state ^= self.rabin.preset.out_table[1];
            } else {
                self.state ^= self.rabin.preset.out_table[0];
            }
            self.state = (self.state << 8)
                | (self.buf[self.cur_idx] as u64)
                    ^ self.rabin.preset.mod_table[self.state as usize >> 45];
            self.cur_idx += 1;
        }

        // cycle
        while self.cur_idx < self.next_round_max
            && ((self.state & self.rabin.mask) != self.rabin.config.target_value)
        {
            self.state ^= self.rabin.preset.out_table
                [self.buf[self.cur_idx - self.rabin.preset.window_size] as usize];
            self.state = (self.state << 8)
                | (self.buf[self.cur_idx] as u64)
                    ^ self.rabin.preset.mod_table[self.state as usize >> 45];
            self.cur_idx += 1;
        ...
        let (chunk, rest) = self.buf.split_at(self.cur_idx - self.last_idx);
        self.buf = rest;
        Some(chunk)
    }
}
...
mod tests {
    ...
    fn test_simple() {
        ...
        let split = chunker.split(&data, true);
        let result: Vec<&[u8]> = split.collect();
        assert_eq!(result.len(), 1);
        assert_eq!(result[0], &data);

        let split = chunker.split(&data, false);
        let result: Vec<&[u8]> = split.collect();
